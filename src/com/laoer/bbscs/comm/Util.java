...
public class Util {
	...
	//积分系统链接地址
	private static String LINK_INTEGRAL_URL = rbint.getString("linkIntegralURL");
	 ...
	 * @throws IOException 
	 */
	public static String getResultMessage(String url, String param) throws IOException {
//        try{
...
//        }
        ...
        logger.info("put-url:"+url+"\n"+httpUrl.getURL()+param+"\n");
		httpUrl.connect();
		httpUrl.getOutputStream().write(param.getBytes("UTF-8"));
	...
	public static JSONObject getUserInfo(HttpServletRequest request){
		...
		if(jsessionid != null){
			try {
				return JSONObject.fromObject(getResultMessage(usersCenterUrl+"user/center!getUserInfo;jsessionid="+jsessionid+"?","a="+Math.random()));
			} catch (IOException e) {
	 ...
	 * @return 发生异常将返回 null  正确取值 将返回一个json对象 用户已经登录 在errorCode 会返回0
	 */
	public static String getUserHeadPath(HttpServletRequest request){
		
		String jsessionid = getJsessionIdToCookies(request);
		if(jsessionid != null){
			try {
				String headpath = "";
				if(JSONObject.fromObject(getResultMessage(usersCenterUrl+"user/center!getUserInfo;jsessionid="+jsessionid+"?","a="+Math.random())).getString("errorCode").equals("0")){
					headpath = JSONObject.fromObject(getResultMessage(usersCenterUrl+"user/center!getUserInfo;jsessionid="+jsessionid+"?","a="+Math.random())).getJSONObject("value").getString("headpath");
				}
				return headpath;
			} catch (IOException e) {
				logger.info("用户头像信息查询出现异常");
			}
		}
		return null;
	}
	...
	public static JSONObject getUserNo(HttpServletRequest request){
		...
		if(jsessionid != null){
			try {
				return JSONObject.fromObject(getResultMessage(usersCenterUrl+"user/center!getUserNo;jsessionid="+jsessionid+"?","a="+Math.random()));
			} catch (IOException e) {
				logger.info("获取用户信息出现异常");
			}
		}
		return null;
	}
	...
	public static void logoutUser(HttpServletRequest request){
		...
		if(jsessionid != null){
			try {
				getResultMessage(usersCenterUrl+"user/center!logout;jsessionid="+jsessionid+"?","a="+Math.random());
			} catch (IOException e) {
	 ...
	 * @return 发生异常将返回 null  正确取值 将返回一个json对象 用户已经登录 在errorCode 会返回0
	 */
	public static Long getUserIntegral(HttpServletRequest request,String userno){
			try {
				JSONObject userIntegral = JSONObject.fromObject(getResultMessage(LINK_INTEGRAL_URL+"/findScoreByUserno?","userno="+userno));
				if(userIntegral != null && !userIntegral.isNullObject() && "0".equals(userIntegral.getString("errorCode"))){
					return userIntegral.getJSONObject("value").getLong("score");
				}else {
					return 1L;
				}
			} catch (IOException e) {
				logger.info("用户积分查询出现异常");
			}
		return 1L;
	}
	...
	public static void main(String[] args) {
		System.out.println(Util.hash("123123"));
	}
}
